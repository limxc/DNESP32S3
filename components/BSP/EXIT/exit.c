...
void exit_init(void)
{
    ...
    ret = gpio_config(&gpio_init_struct);                      /* 配置使能 */
    if (ret != ESP_OK)
    {
        ESP_LOGE("EXIT", "Failed to configure GPIO: %s", esp_err_to_name(ret));
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE("EXIT", "Failed to install ISR service: %s", esp_err_to_name(ret));
    }

    /* 设置GPIO的中断回调函数 */
    ret = gpio_isr_handler_add(BOOT_INT_GPIO_PIN, exit_gpio_isr_handler, (void *)BOOT_INT_GPIO_PIN);
    if (ret != ESP_OK)
    {
        ESP_LOGE("EXIT", "Failed to add ISR handler: %s", esp_err_to_name(ret));
    }
}
